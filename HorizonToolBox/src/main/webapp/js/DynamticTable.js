			...
			app:  angular.module('main', ['ngTable']).controller('usageCtrl', function($scope, ngTableParams) {
				$scope.days = "7";
				 $scope.$watch("days", function () {
				    ...
				    $scope.remove = function(tobeRemoved){
						var index = -1;
						for (var i=0;i<ToolBox.Usage.usageData.length;i++){
							if (ToolBox.Usage.usageData[i] == tobeRemoved){
								index = i;
								break;
							}
						}
						...
						if (index>=0){
							for (var i = index; i<ToolBox.Usage.usageData.length -1 ;i++){
								ToolBox.Usage.usageData[i] = ToolBox.Usage.usageData[i+1];
							}
							ToolBox.Usage.usageData.pop();
							 $scope.tableParams.reload();
						}
			    ...
			    $scope.add = function(){
					 ToolBox.Usage.usageData.push({username: "faked", connectionTime: new Date().toLocaleString()});
					 $scope.tableParams.reload();
				 };
