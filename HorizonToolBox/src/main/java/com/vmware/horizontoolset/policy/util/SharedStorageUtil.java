...
public class SharedStorageUtil {
	...
	private boolean checkSaveSuccess(String keyName, String value){
		String res = ToolboxStorage.getStorage().get(keyName);
		if( res.equals(value) )
			return true;
		return false;
	}
	...
	private void sharedStorageInit(){
		...
		if( null==indexRes ){
			...
			ToolboxStorage.getStorage().set(SharedStorageIndex_Name, indexListStr);
		}
		String countMapRes = get_(SubItemsCountTable_Name);
		if( null==countMapRes ){
			Map<String, String> countMap = new HashMap<String, String>();
			String countMapStr = JsonUtil.javaToJson(countMap);
			ToolboxStorage.getStorage().set(SubItemsCountTable_Name, countMapStr);
		}
	}

	private List<String> getIndexList(){
		String indexListStr = ToolboxStorage.getStorage().get(SharedStorageIndex_Name);
	...
	private boolean set_(String key, String value){
		ToolboxStorage.getStorage().set(key, value);
	...
	private String get_(String key){
		return ToolboxStorage.getStorage().get(key);
	}

	private void delete_(String key){
		ToolboxStorage.getStorage().delete(key);
		deleteFromIndex(key);
	}
